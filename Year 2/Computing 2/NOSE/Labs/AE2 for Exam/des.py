...
class SchedulerDES(object):
    ...
    def generate_and_init(self, seed=None):
        ...
        random.seed(seed)
        cur_time = 0
        exam_processes = [(0,5), (3,9), (4,15), (6,10), (8,11)]
        for i in range(len(exam_processes)):
            cur_time = exam_processes[i][0]
            new_process = Process(process_id=i, arrival_time=cur_time, service_time=exam_processes[i][1])
