...
public:
  void safe_push_back(char *name) {
    std::unique_lock<std::mutex> lock(mutex);
    queue.push_back(name);
  }

  std::optional<std::string> safe_pop_front() {
    std::unique_lock<std::mutex> lock(mutex);
    if (queue.size() > 0) {
...
public:
  bool safe_insert_new_name(char *name) {
    std::unique_lock<std::mutex> lock(mutex);
    // 2bii. if file name not already in table ...
    if (table.find(name) != table.end()) {
      return false; // name was not inserted
    }
  ...
  bool safe_insert(std::pair<std::string, std::list<std::string>> value) {
    std::unique_lock<std::mutex> lock(mutex);
    auto res = table.insert(value);
    return res.second;
  }

  std::list<std::string> *safe_get(std::string name) {
    std::unique_lock<std::mutex> lock(mutex);
    return &table[name];
  }

  int safe_find(const char *name) {
    std::unique_lock<std::mutex> lock(mutex);
    if (table.find(name) == table.end()) {
      return -1;
    }
...
static void process(const char *file, std::list<std::string> *ll) {
  ...
  while (fgets(buf, sizeof(buf), fd) != NULL) {
    ...
    *q = '\0';
    ...
    // 2bii. if file name not already in table ...
    // ... insert mapping from file name to empty list in table ...
    auto inserted = theTable.safe_insert_new_name(name);
    if (inserted) {
      // ... append file name to workQ (only if the name has been inserted)
...
static void printDependencies(std::unordered_set<std::string> *printed,
  ...
  while (toProcess->size() > 0) {
    ...
    std::list<std::string> *ll = theTable.safe_get(name);
...
int main(int argc, char *argv[]) {
  ...
  int crawler_nb = crawler ? std::stoi(crawler) : 2;
  ...
  for (int i = 0; i < crawler_nb; ++i) {
    ts.emplace_back([&]() { // launch new thread
      // 4. for each file on the workQ
      auto item = workQ.safe_pop_front();
      while (item.has_value()) {
        if (theTable.safe_find(item.value().c_str()) == -1) {
          fprintf(stderr, "Mismatch between table and workQ\n");
          return -1;
        }

        // 4a&b. lookup dependencies and invoke 'process'
        process(item.value().c_str(), theTable.safe_get(item.value()));
        item = workQ.safe_pop_front();
      }
      return 0;
    });
  }
