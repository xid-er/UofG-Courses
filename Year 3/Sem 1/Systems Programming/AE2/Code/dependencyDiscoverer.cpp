 ...
 * 	SP Exercise 2a
 * 	This is my own work except that I got inspiration for the vector of threads
 *  (specifically, moving threads and checking if they're joinable) from
...
struct safeMap{
	...
	public:
		...
		void insert(std::pair<const std::string,std::list<std::string>> il){
		  std::unique_lock<std::mutex> lock(mutex);
		  theTable.insert(il);
		}
...
auto safeTable = safeMap{};
auto safeWorkQ = safeQ{};
...
static void process(const char *file, std::list<std::string> *ll) {
  ...
  while (fgets(buf, sizeof(buf), fd) != NULL) {
    ...
    *q = '\0';
    ...
    if (safeTable.find(name) != safeTable.end()) { continue; }
    // ... insert mapping from file name to empty list in table ...
    safeTable.insert( { name, {} } );
    // ... append file name to workQ
    safeWorkQ.push_back( name );
  }
  // 3. close file
  fclose(fd);
}
...
static void printDependencies(std::unordered_set<std::string> *printed,
  ...
  while ( toProcess->size() > 0 ) {
    ...
    std::list<std::string> *ll = &safeTable[name];
...
int main(int argc, char *argv[]) {
  ...
  for (i = start; i < argc; i++) {
    ...
    safeTable.insert( { obj, { argv[i] } } );
    
    // 3b. insert mapping from file.ext to empty list
    safeTable.insert( { argv[i], { } } );
    
    // 3c. append file.ext on workQ
    safeWorkQ.push_back( argv[i] );
  }

  // 4. for each file on the workQ

  // Get no. of threads from environment, else 2
  char *no_threads_char_arr = getenv("CRAWLER_THREADS");
  int no_threads;
  if (no_threads_char_arr != NULL) {
    try {
      no_threads = std::stoi(std::string(no_threads_char_arr));
    }
    catch(std::invalid_argument& e) {
      fprintf(stderr, "Not a valid no. of threads\n");
      return -1;
    }
  ...
  // Build a vector with required amount of threads
  std::vector<std::thread> threads(no_threads);
  int error = 0;
  for (int i = 0; i < no_threads; i++) {
    // Pass in the thread-safe table, workQ and an error variable for catching mismatches
    auto worker = std::thread([q=&safeWorkQ, t=&safeTable, error=&error](){
      while ( q->size() > 0 ) {
        // Thread gets a job (if only it were this easy)
        std::string filename = q->front();
        q->pop_front();

        if (t->find(filename) == t->end()) {
          fprintf(stderr, "Mismatch between table and workQ\n");
          *error = -1;
        }

        // 4a&b. lookup dependencies and invoke 'process'
        process(filename.c_str(), &(*t)[filename]);
      }
    });
    threads.push_back(std::move(worker));
  }

  // Join all threads in the vector
