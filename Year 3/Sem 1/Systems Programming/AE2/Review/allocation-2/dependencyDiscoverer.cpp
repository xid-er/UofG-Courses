...
// // stores a list of directories to search for headers
// std::vector<std::string> dirs;
// // maps file names to a list of dependent file names
// std::unordered_map<std::string, std::list<std::string>> theTable;
// // a list of file names that have to be processed
// std::list<std::string> workQ;

//Create all the thread safe data structures to encapsulate dir, theTable and workQ
struct ts_dirs{
  private:
  std::vector<std::string> dirs;
  std::mutex dmutex;
...
struct ts_theTable{
  ...
  public:
  auto find(std::string i){
    std::unique_lock<std::mutex> lock(tmutex);
    return theTable.find(i);
  }
  auto end(){
    std::unique_lock<std::mutex> lock(tmutex);
    return theTable.end();
  }
  void insert(std::pair<std::string, std::list<std::string>> pairData){
    std::unique_lock<std::mutex> lock(tmutex);
    theTable.insert(pairData);
  }

  //to resolve the subscript operator error
  auto getAddress(std::string name){
    std::unique_lock<std::mutex> lock(tmutex);
...
// Define the Data Structures in thread safe versions
ts_dirs dirs;
ts_theTable theTable;
ts_workQ workQ;
...
//breaks up filename into root and extension
...
int main(int argc, char *argv[]) {
  ...
  // Check the Crawlers Threads
  char *crawlersThreads = getenv("CRAWLER_THREADS");
  int crawlersThreadCount;

  if (crawlersThreads ==NULL){
    crawlersThreadCount = 2;
  }
  else{
    crawlersThreadCount = strtoul(crawlersThreads, NULL,10 );
  }

  //std::cout << "This is a test " << crawlersThreadCount << "\n";
  ...
  // Create a container to hold all the threads
  std::vector <std::thread> allThreads;
  for (int i = 0; i <crawlersThreadCount;i++){
    // push_back gave an error while adding a lambda into the vector
  ...
  // Make sure that all the threads are executed. 
