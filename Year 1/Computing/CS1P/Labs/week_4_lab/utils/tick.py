...
def marks(marks):
    ...
    try:
        ...
        IPython.display.display(
            IPython.display.HTML(
                '<h3> <!--{id:"CORRECTMARK", marks:"%d"}--> <font color="green"> ✓ [%d marks] </font> </h3>'
                % (marks, marks)
            )
        )
        total_marks += marks
        ...
        IPython.display.display(
            IPython.display.HTML(
                '<hr style="height:10px;border:none;color:#f00;background-color:#f00;" /><h3> <!--{id:"WRONGMARK", marks:"%d"}--> <font color="red"> Test failed ✘ [0/%d] marks </font> </h3>'
                % (marks, marks)
            )
        )
        raise e
